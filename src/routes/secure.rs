...
/*
Validate a session token without invalidating the token after the request is finished. Used for
raw data requests generated by a processing response, which must make all requests with the same
token.
 */
pub fn session_without_invalidation<T, S: SessionStore>(
    session_store: S,
) -> impl Fn(T) -> BoxedFilter<(Box<dyn Reply>,)>
    where
        T: Filter<
            Extract = (Box<dyn Reply>,),
            Error = Rejection,
        > + Clone
        + Send
        + Sync
