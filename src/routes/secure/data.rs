...
use redact_crypto::{BinaryData, BinaryType, CryptoError, Data, Storer};
...
use crate::{
    relayer::Relayer,
    render::Renderer,
    routes::{validated_query_params, BadRequestRejection, CryptoErrorRejection},
    token::TokenGenerator,
};

pub fn get<R: Renderer, H: Storer, T: TokenGenerator>(
    storer: Arc<H>,
    render_engine: R,
    token_generator: T,
        ...
        .and_then(
            |path: String,
             ...
             render_engine: R| async move {
                ...
                Ok::<_, Rejection>((
                    get::reply(data, &path, &new_token, query, render_engine)?,
...
// pub fn post<R: Renderer, T: TokenGenerator, S: Storer, Q: Relayer>(
//     render_engine: R,
//     token_generator: T,
//     storer: S,
//     relayer: Q,
// ) -> impl Filter<Extract = (impl Reply, String, Option<String>, Option<String>), Error = Rejection> + Clone
// {
//     warp::post().and(warp::path!(String)).and(
